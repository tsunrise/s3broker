...
export async function evaluateGuardrails(
    ...
    const config: GuardrailConfig = defaultConfig;
    const path = new URL(request.url).pathname;
    const policies = getPolicies(config, path, upstreamFetcher, env.S3_ENDPOINT, currentTimestampMs);
